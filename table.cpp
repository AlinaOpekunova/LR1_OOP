...
void Table::setCell(const int x, const int y, Cell &&cell_) { // констуктор копирования
    if ((x >= 0) && (x < row) && (y >= 0) && (y < col)){
        cell[x][y] = std::move(cell_);
    }
    else
...
void Table::loadTable(const std::string filename) { // метод, принимающий путь к файлу и загружающий из него содержимое коллекции
    double n, m;
    std::ifstream file;
    ...
    if (!file)
        ...
        while(1){
            ...
            for (int i=0; i<n;i++){
                ...
                for (int j=0; j<m; j++){
                    file >> str;
                    setCell(i,j,Cell(0.0, str, CellType::TEXT));
                }
            }
